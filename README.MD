...
- **Web admin panel**: Browser-based UI at `/admin` — dashboard (user/item counts), user management (add, change role, delete), paginated item browser with search and availability filter, bot settings editor (persists to `config.json` or `.env`), **VPN management** tab (amnezia-wg-easy: list clients with traffic stats, create/delete clients, enable/disable, download `.conf` and QR code). Protected by username/password login with httponly JWT cookie.
...
app/
  ...
  services/
    ...
    vpn_client.py      # Async HTTP client for wg-easy/amnezia-wg-easy API (auth, client CRUD, QR/config proxy)
  schemas/
  ...
  templates/           # Jinja2 HTML templates
    ...
    items.html         # Paginated item browser
    settings.html      # Bot settings editor
    vpn.html           # VPN client management
...
tests/
  ...
  test_encryption.py   # encrypt_value, decrypt_value (round-trip, empty/invalid key or cipher)
  ...
  test_app_repository.py   # get_item_by_id, update_item, delete_by_name_and_chat, create_item, get_items
  test_app_user_repository.py  # get_by_telegram_id, list_users, create_user, set_role, delete_user
  test_schemas.py      # ItemCreate, ItemUpdate
  test_admin.py        # Admin panel JWT, middleware, login/logout, dashboard, users CRUD, items, settings
  test_vpn.py          # VPN: _fmt_bytes, VPNClient (auth, all methods, lazy auth, 401 re-auth), get_vpn_client factory, all VPN admin routes
  playwright/          # Playwright E2E and API tests (require a running app instance)
    conftest.py        # Shared fixtures: api_request_context, admin_credentials, authenticated_page
    pages/
      base_page.py     # BasePage — navigate(), current_url, title
      login_page.py    # LoginPage — locators for /admin/login
      dashboard_page.py # DashboardPage — nav links, stat cards, logout
    smoke/
      test_smoke.py    # @smoke — app availability, login page load, login flow
    api/
      test_health.py   # @regression — /webhook/health, /, /webhook/telegram
      test_admin.py    # @regression — admin routes, auth, user management endpoints
```
...
- Admin panel: `https://your-domain.com/admin` (requires HTTPS and `WEB_ADMIN_PASSWORD` in `.env`)
...
| `DATABASE__*` or `DATABASE` | DB name, user, password, host, port, table_name; optional `DATABASE__POOL_SIZE` (default 5), `DATABASE__MAX_OVERFLOW` (default 10) |
...
| `WEB_ADMIN_USER` | Admin panel login username (default: `admin`) |
| `WEB_ADMIN_PASSWORD` | Admin panel password — **must be set**; panel login is disabled if empty |
| `WEB_ADMIN_JWT_SECRET` | JWT cookie secret; set to a fixed value so sessions survive restarts (random per-restart if unset) |
| `VPN_API_URL` | Base URL of the amnezia-wg-easy API, e.g. `http://localhost:51821` (optional; VPN tab hidden if empty) |
| `VPN_API_PASSWORD` | Password for the wg-easy web UI (optional) |
...
Run all 205 unit tests (no running app required):

```bash
poetry run pytest          # legacy test_bot.py, test_database.py, and tests/playwright/ are excluded automatically
poetry run pytest -v       # verbose output
```

`test_bot.py` and `test_database.py` import removed legacy classes and are excluded via `pyproject.toml`.
Encryption tests (`test_encryption.py`) are skipped if `cryptography` is not installed.

### Playwright E2E and API tests

These tests run against a **live app instance** (local or remote).

**One-time setup** (install browsers after `poetry install`):
...
**Run smoke tests** (fast sanity checks — health endpoint, login page, auth flow):

```bash
poetry run pytest tests/playwright/ -m smoke -v
```

**Run regression tests** (full API coverage — all admin routes, input validation, auth):

```bash
poetry run pytest tests/playwright/ -m regression -v
```

**See the browser** (add `--headed` to any Playwright run):

```bash
poetry run pytest tests/playwright/ -m smoke -v --headed
```

**Override the target URL or credentials:**

```bash
# Different environment
poetry run pytest tests/playwright/ --base-url https://staging.example.com -v

# Custom admin credentials
WEB_ADMIN_USER=myadmin WEB_ADMIN_PASSWORD=secret poetry run pytest tests/playwright/ -v
```
...
4. **Indexes**  
   The app ensures performance indexes on `organizer_table` on every startup (`ensure_indexes`). No extra step needed on deploy or redeploy — they are created automatically and idempotently (IF NOT EXISTS). You can also run the SQL by hand: `psql -U botuser -d telegram_bot -f SQL_queries/Add_indexes.sql`.

   **How indexes work:** Indexes are stored in the database (not in app code). They speed up queries by letting PostgreSQL find rows by `chat_id`, `(chat_id, timestamp)`, or `(chat_id, item_name)` without scanning the whole table. Once created, they persist across app restarts and redeploys. The app runs `CREATE INDEX IF NOT EXISTS` on startup so indexes are created if missing and left as-is if they already exist.
...
- More: [DEPLOYMENT.md § Troubleshooting](DEPLOYMENT.md#9-troubleshooting).
...
- **`/docs`** – Interactive Swagger UI (available only when `DEBUG=true`).
- **`/redoc`** – ReDoc API reference (available only when `DEBUG=true`).
